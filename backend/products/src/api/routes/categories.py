...
async def get_categories(repo_manager: RepositoryManager = Depends(get_repository_manager)):
    return await repo_manager.category.get_all()


@category_router.get("/{category_id}", response_model=CategoryRead)
async def get_category(category_id: int, repo_manager: RepositoryManager = Depends(get_repository_manager)):
...
async def get_category_by_name(category_name: str, repo_manager: RepositoryManager = Depends(get_repository_manager)):
...
async def create_category(
    category_data: CategoryCreate, repo_manager: RepositoryManager = Depends(get_repository_manager)
):
...
async def update_category(
    category_id: int, category_data: CategoryUpdate, repo_manager: RepositoryManager = Depends(get_repository_manager)
):
...
async def delete_category(category_id: int, repo_manager: RepositoryManager = Depends(get_repository_manager)):
