...
async def get_comments(repo_manager: RepositoryManager = Depends(get_repository_manager)):
    return await repo_manager.comment.get_all()


@comment_router.get("/{comment_id}", response_model=CommentRead)
async def get_comment(comment_id: int, repo_manager: RepositoryManager = Depends(get_repository_manager)):
...
async def create_comment(
    comment_data: CommentCreate, repo_manager: RepositoryManager = Depends(get_repository_manager)
):
...
async def update_comment(
    comment_id: int, comment_data: CommentUpdate, repo_manager: RepositoryManager = Depends(get_repository_manager)
):
...
async def delete_comment(comment_id: int, repo_manager: RepositoryManager = Depends(get_repository_manager)):
