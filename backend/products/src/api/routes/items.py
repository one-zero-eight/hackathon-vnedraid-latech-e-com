...
async def create_item_in(
    item_data: ItemInCreate,
    repo_manager: RepositoryManager = Depends(get_repository_manager)
):
    return await repo_manager.item_in.create(item_data)

@item_in_router.put("/{item_id}", response_model=ItemInRead)
async def update_item_in(
    item_id: int,
    item_data: ItemInUpdate,
    repo_manager: RepositoryManager = Depends(get_repository_manager)
):
...
async def create_item_out(
    item_data: ItemOutCreate,
    repo_manager: RepositoryManager = Depends(get_repository_manager)
):
    return await repo_manager.item_out.create(item_data)

@item_out_router.put("/{item_id}", response_model=ItemOutRead)
async def update_item_out(
    item_id: int,
    item_data: ItemOutUpdate,
    repo_manager: RepositoryManager = Depends(get_repository_manager)
):
